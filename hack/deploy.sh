...
PID=$(ssh $REMOTE_USER@$REMOTE_HOST "pgrep -f $REMOTE_JAR_NAME")
echo "Find PID: $PID"
...
ssh $REMOTE_USER@$REMOTE_HOST "nohup /usr/jdk/bin/java -jar $REMOTE_PATH/$REMOTE_JAR_NAME > /root/springboot.log 2>&1"
